...
# the results from running "simulation_code_02152023.R"
rm(list=ls())
...
## need to run the following chunk of codes first to make plots.  
################
#############
library(proto)
...
# 1) for p=5 and g = nonlinear  # 7, 8, 9
...
Method <- rep(factor(c(rep("Index model", n.rep), rep("Linear model", n.rep), rep("True optimal", n.rep)),
                     levels = c("Index model","Linear model", "True optimal")), 3)
n <- factor(c(rep("500", 3*n.rep), rep("1000", 3*n.rep), rep("2000", 3*n.rep)),
            levels = c("500", "1000", "2000"))
data1 <- data.frame(Value, Method, n)


# 2) for p=10 and g = nonlinear # 10, 11, 12
...
Method <- rep(factor(c(rep("Index model", n.rep), rep("Linear model", n.rep)),
                     levels = c("Index model","Linear model")), 3)
